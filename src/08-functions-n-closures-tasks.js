 ...
 */
function memoize(func, prevValue) {
  if (prevValue) {
    return () => prevValue;
  }
  const tmp = func();
  return memoize(func, tmp);
}
...
function logger(/* func, logFunc */) {
  // const funcName = func.toString().replace('function ', '').split('(')[0];
  // return (...args) => {
  //   let vars = '';
  //   for (let i = 0; i < args.length; i += 1) {
  //     if (typeof args[i] === 'object') {
  //       vars += JSON.stringify(args[i]);
  //     } else {
  //       vars += args[i].toString();
  //     }
  //     if (i !== args.length - 1) {
  //       vars += ',';
  //     }
  //   }
  //   logFunc(`${`${funcName}(${vars}`}) starts`);
  //   const result = func(args);
  //   logFunc(`${`${funcName}(${vars}`}) ends`);
  //   return result;
  // };
