...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(charAti) === i && str.indexOf(charAti, i + 1) === -1) {
      return charAti;
    }
  }
  return null;
...
function getDigitalRoot(num) {
  let numb = num;
  while (numb > 9) {
    const numToArray = String(numb)
      .split('')
      .map((element) => Number(element));
    numb = numToArray.reduce((acc, cur) => acc + cur, 0);
  }
  return numb;
}
...
function getCommonDirectoryPath(pathes) {
  const num = pathes[0].length;
  for (let i = num - 1; i >= 0; i -= 1) {
    // eslint-disable-next-line no-continue
    if (pathes[0][i] !== '/') continue;
    if (pathes.every((p) => p.startsWith(pathes[0].slice(0, i + 1)))) {
